          ...
          {FEATURES_GRID_DATA?.map(
            ({ id, title, description, image }, index) => {
              return (
                ...
                >
                  <div
                    className={`rounded-3xl overflow-hidden bg-[#15131d] flex relative ${featuredGridSettings[index].cardFlexCol} ${featuredGridSettings[index].cardGradientAfter} ${featuredGridSettings[index].cardGradientBefore} justify-between items-center h-full lg:gap-y-[26px] md:gap-y-5 gap-y-4`}
                  >
